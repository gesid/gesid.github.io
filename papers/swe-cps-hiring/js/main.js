...
function filterPlaybook(searchTerm) {
  ...
  for (const phaseName in appState.playbookStructure) {
    ...
    for (const themeName in themes) {
      const filteredApIds = themes[themeName].filter(cpId => {
        const pattern = appState.patternsMap.get(cpId);
        if (!pattern) return false;
        const content = `${pattern.id} ${pattern.name} ${pattern.description}`.toLowerCase(); // Adicionada descrição à busca
        if (content.includes(term)) return true;
        const quotes = appState.quotesMap[cpId] || {};
...
function renderPlaybook(structure, searchTerm) {
  ...
  for (const [phase, themes] of Object.entries(structure)) {
    ...
    phaseSection.innerHTML = `<h2 class="text-3xl font-semibold text-green-500 mb-8 border-b-2 border-green-500/20 pb-2">${phase}</h2>`;
    const themesContainer = document.createElement('div');
    themesContainer.className = 'space-y-12';

    for (const [themeFullName, cpIds] of Object.entries(themes)) {
      ...
      cpIds.forEach(cpId => grid.appendChild(createCardElement(cpId, themeCode, searchTerm)));
...
function createCardElement(cpId, themeCode, searchTerm) {
  const pattern = appState.patternsMap.get(cpId);
  const card = document.createElement('div');
  card.className = "bg-zinc-900 border border-zinc-800 rounded-lg p-5 cursor-pointer hover:bg-zinc-800 hover:border-green-500/50 transition-all duration-200 h-full flex flex-col";
  
  const nameHtml = highlightText(pattern.name, searchTerm);
  const descriptionHtml = highlightText(pattern.description || '', searchTerm); // Adicionado

  card.innerHTML = `
    <p class="text-sm font-semibold text-green-500">${pattern.id}</p>
    <h4 class="text-lg font-bold text-slate-100 mt-2">${nameHtml}</h4>
    <p class="text-slate-400 mt-3 text-sm flex-grow">${descriptionHtml}</p> <!-- Adicionado -->
  `;
  
  card.addEventListener('click', () => openModal(cpId, themeCode, searchTerm));
  return card;
}
...
function highlightText(text, highlight) {
    ...
    return text.replace(regex, `<mark class="bg-green-500/30 text-slate-100 px-1 rounded-sm">$1</mark>`);
}
...
function renderSummaryList(patterns) {
    ...
    container.innerHTML = patterns
        ...
        .map(p => `<div><h3 class="font-bold text-slate-100"><span class="text-green-500 font-semibold">${p.id}:</span> ${p.name}</h3><p class="text-slate-400 mt-1 text-sm">${p.description}</p></div>`).join('');
}
...
function openModal(cpId, themeCode, searchTerm) {
    const pattern = appState.patternsMap.get(cpId);
    const header = document.getElementById('modal-header');
    const body = document.getElementById('modal-body');

    header.innerHTML = `
        <h2 class="text-2xl text-green-500">${pattern.id}: ${pattern.name}</h2>
        <p class="text-slate-400 pt-2 text-base">${pattern.description}</p> <!-- Adicionado -->
    `;
    
    const allQuotesForCP = appState.quotesMap[cpId] || {};
    const quotesForTheme = allQuotesForCP[themeCode] || []; 
    ...
    if (quotesForTheme.length > 0) {
        ...
        quotesForTheme.forEach(quote => { 
            ...
            quotesHTML += `<blockquote class="border-l-4 border-green-500/50 pl-4 italic text-slate-300"><p class="mb-2">&ldquo;${highlightText(text, searchTerm)}&rdquo;</p><footer class="text-right text-sm text-slate-500 not-italic">- ${author}</footer></blockquote>`;
        });
        quotesHTML += '</div>';
    } else {
