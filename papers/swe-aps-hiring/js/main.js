...
function setupSearchListener() {
  ...
  searchInput.addEventListener('input', (e) => {
    ...
    const filteredStructure = filterPlaybook(searchTerm);
    renderPlaybook(filteredStructure, searchTerm);
  });
}
...
function filterPlaybook(searchTerm) {
  ...
  for (const phaseName in appState.playbookStructure) {
    ...
    for (const themeName in themes) {
      const apIds = themes[themeName];
      const filteredApIds = apIds.filter(apId => {
    ...
    if (phaseHasResults) {
      newStructure[phaseName] = filteredThemes;
    }
  }
  return newStructure;
}
  ...
  for (const [phase, themes] of Object.entries(structure)) {
    ...
    for (const [themeFullName, apIds] of Object.entries(themes)) {
      ...
      themeContainer.innerHTML = `
        <h3 class="text-xl font-medium text-slate-300 mb-2">${themeName}</h3>
        <p class="text-slate-400 max-w-4xl mb-6 text-base italic">${themeDescription}</p>
      `;
        ...
        .map(p => `
            <div>
                <h3 class="font-bold text-slate-100">
                    <span class="text-red-500 font-semibold">${p.id}:</span> ${p.name}
                </h3>
            </div>`
        ).join('');
}
...
function createModalElements() {
    const modalHTML = `
        ...
        <div id="modal-content" class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-zinc-900 border-zinc-700 text-slate-200 max-w-3xl w-full max-h-[90vh] overflow-y-auto rounded-lg p-6 z-50 hidden">
            ...
            <div id="modal-body" class="my-6"></div>
        </div>
    `;
...
function openModal(apId, themeCode, searchTerm) {
    ...
    if (quotesForTheme.length > 0) {
        ...
        quotesForTheme.forEach(quote => {
            ...
            quotesHTML += `
                <blockquote class="border-l-4 border-red-500/50 pl-4 italic text-slate-300">
                    <p class="mb-2">&ldquo;${highlightText(text, searchTerm)}&rdquo;</p>
                    <footer class="text-right text-sm text-slate-500 not-italic">- ${author}</footer>
                </blockquote>
            `;
        });
        quotesHTML += '</div>';
